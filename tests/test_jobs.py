...
class TestJobIdManagement:
    ...
    def test_get_ids_invalid_type(self):
        ...
        job = Job()
        with pytest.raises(
            TypeError, match="IDs must be a string or a list of strings"
        ):
            job._get_ids(123, 1)
    ...
    def test_get_ids_empty(self):
        ...
        job = Job()
        with pytest.raises(
            ValueError, match="Number of chains or ligands must be greater than zero"
        ):
            job._get_ids([], 0)
...
class TestJobAddConstraints:
    ...
    def test_add_disulfide_bond_non_cysteine(self, sample_protein_sequence):
        ...
        cys_seq = sample_protein_sequence.replace("M", "C")
        job = Job()
        job.add_protein_chain(sample_protein_sequence, ids="A")
        job.add_protein_chain(cys_seq, ids="B")

        with pytest.raises(ValueError, match="is not a cysteine"):
            # first chain position 1 is M, not C
            job.add_disulfide_bond("A", 1, "B", 1)

        with pytest.raises(ValueError, match="is not a cysteine"):
            # second chain position 1 is M, not C
            job.add_disulfide_bond("B", 1, "A", 1)

    @pytest.mark.unit
    def test_add_disulfide_bond_same_residue(self, sample_protein_sequence):
        """Test adding disulfide bond to same residue raises error."""
        cys_seq = sample_protein_sequence.replace("M", "C")
        job = Job()
        job.add_protein_chain(cys_seq, ids="A")

        with pytest.raises(
            ValueError, match="Cannot form a disulfide bond with the same residue."
        ):
            # both chain and residue are the same
            job.add_disulfide_bond("A", 1, "A", 1)

    @pytest.mark.unit
    def test_add_disulfide_bond_invalid_chain_ids(self):
        """Test adding disulfide bond with invalid chain IDs."""
        job = Job()
        with pytest.raises(
            ValueError, match="One or both chain IDs not found as ProteinChain."
        ):
            job.add_disulfide_bond("X", 1, "Y", 1)
